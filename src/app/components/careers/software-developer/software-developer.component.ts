...
export class SoftwareDeveloperComponent implements OnInit, OnDestroy {
  ...
  private applyMetaTags() {
    try {
      const lang = this.i18n.getLang();
      const name = this.i18n.t('data.name') || 'Pedro Carpio';
      const roleTitle = lang === 'es' ? 'Frontend Developer & UX Designer Jr' : 'Frontend Developer & UX Designer Jr';
      const summaryText = (this.i18n.t('data.softwareDeveloper.summaryText') as string) || '';
      const image = 'https://kusillo.studio/kusillo.webp';
      const url = this.isBrowser ? window.location.href : 'https://kusillo.studio/yo-como/software-developer';
  ...
  private loadRoles(): void {
    const val = this.i18n.t('data.softwareDeveloper.rolesList') as any;
